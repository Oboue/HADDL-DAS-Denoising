function [dn,ds] = pwsmooth_lop_mf(adj,add,dip,w1,n1,n2,ns,order,eps,ndn,nds,type_mf,ifsmooth,dn,ds)
% pwsmooth_lop: Plane-wave smoothing and median filtering (benchmarked with Madagascar, exact
% the same)
%
...
if adj %adjoint operator deactivated
    ...
    for i2=1:n2
        if type_mf==0   %MF
        u(:,:,i2)= yc_mf(u(:,:,i2),ns2,1,2);
        else            %SVMF
        u(:,:,i2)= svmf0(u(:,:,i2),ns2,1,2);    
        end
    end
