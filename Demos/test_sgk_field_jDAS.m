...
% Pre-calcul du residu
removed_noise = jdas_data - sgkjdas;

% Stockage dans une cellule pour la boucle
data_plot = {jdas_data, sgkjdas, removed_noise};
plot_titles = {'Noisy Input', 'Denoised (BP+SGK Baseline)', 'Removed Noise (Residual)'};

for k = 1:3
    subplot(1, 3, k);
    
    % --- CORRECTION : Transposition data_plot{k}' pour alignement Y=Time, X=Distance ---
    imagesc(distance/1000, time, data_plot{k}'); 
    
    colormap(seis); % ou colormap(gray) selon vos preferences
    caxis([-5 5]);
    colorbar;
