...
% --- MODIFICATION ICI : ajout de la transposition dn' ---
imagesc(distance/1000, time, dn'); % Transposition de la matrice de donnees
...
caxis([-5 5]);
set(gca, 'YDir', 'reverse'); % Ensures t=0 is at the top

% --- Subplot 2: Denoised Data ---
subplot(132);
imagesc(distance/1000, time, d_mharn'); % Transposition d_mharn'
colormap(seis);
colorbar;
xlabel('Distance along cable (km)');
ylabel('Time (s)');
...
%% Save denoised data
filename = fullfile(outputFolder, sprintf('d_mharn_jDAS%d.mat', i));
save(filename, 'd_mharn');
end
%%

% S'assurer que les matrices sont orientées [nt x nx] (Temps en lignes, Canaux en colonnes)
if size(jdas_data, 1) ~= length(time)
    jdas_data_plot = jdas_data';
    d_mharn_plot = d_mharn';
else
    jdas_data_plot = jdas_data;
    d_mharn_plot = d_mharn;
end

[nt, nx] = size(jdas_data_plot);
...
scale = 0.5 * max([abs(jdas_data_plot(:, channels_idx)), abs(d_mharn_plot(:, channels_idx))], [], 'all');

% Plot input and denoised traces
figure('Position', [100 100 1200 900]); 
hold on;

h_in = []; h_den = []; % Handles pour une légende propre

for i = 1:length(channels_idx)
    ch = channels_idx(i);
    
    % Input (black)
    h_in = plot(time, jdas_data_plot(:, ch)/scale + i, 'k', 'LineWidth', 1.2);
    
    % Denoised (red, dashed)
    h_den = plot(time, d_mharn_plot(:, ch)/scale + i, 'r--', 'LineWidth', 1.2);
end
...
% Légende associée uniquement aux objets tracés (évite l'erreur 'P-wave arrival')
legend([h_in, h_den], {'Input', 'Denoised'}, 'Location', 'northeast');
