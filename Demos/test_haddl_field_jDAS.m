...
subplot(131);
imagesc(distance/1000, time, jdas_data'); % Transposition jdas_data'
colormap(seis);
colorbar;
...
% --- Subplot 2: Denoised Data (HADDL) ---
subplot(132);
imagesc(distance/1000, time, d_haddl_jDAS'); % Transposition d_haddl_jDAS'
colormap(seis);
colorbar;
xlabel('Distance along cable (km)');
ylabel('Time (s)');
...
% --- Subplot 3: Removed Noise ---
dn1 = jdas_data - d_haddl_jDAS;
subplot(133);
imagesc(distance/1000, time, dn1'); % Transposition dn1'
colormap(seis);
colorbar;
xlabel('Distance along cable (km)'); % Unification des axes en km
ylabel('Time (s)');                 % Unification des axes en s
title('Removed Noise (Time × Channel)');
caxis([-5 5]);
set(gca, 'YDir', 'reverse', 'LineWidth', 1.5, 'FontSize', 12, 'FontWeight', 'bold');
...
%% BLOC 1: COMPARATIVE TRACES (15 CHANNELS, SHARED SCALE)
% -------------------------------------------------------------------------
% Ensure [nt x nx] orientation for trace extraction
if size(jdas_data, 1) ~= length(time)
    jdas_plot = jdas_data';
    haddl_plot = d_haddl_jDAS';
else
    jdas_plot = jdas_data;
    haddl_plot = d_haddl_jDAS;
end

[nt, nx] = size(jdas_plot);
tmax = 45; % seconds
time = linspace(0, tmax, nt);
...
scale = 0.5 * max([abs(jdas_plot(:, channels_idx)), abs(haddl_plot(:, channels_idx))], [], 'all');

figure('Position', [100 100 1200 900], 'Color', 'w'); 
hold on;

% Handles for clean legend mapping
h_in = []; h_de = [];
for i = 1:length(channels_idx)
    ch = channels_idx(i);
    h_in = plot(time, jdas_plot(:, ch)/scale + i, 'k', 'LineWidth', 1.2);
    h_de = plot(time, haddl_plot(:, ch)/scale + i, 'r--', 'LineWidth', 1.2);
end

% P-wave arrival line
p_wave_time = 12.3; % seconds
h_pw = xline(p_wave_time, '--b', 'LineWidth', 1.5, 'Label', 'P-wave', ...
...
legend([h_in, h_de, h_pw], {'Input', 'Denoised', 'P-wave arrival'}, 'Location', 'northeast');
grid on;
set(gca, 'LineWidth', 1.5, 'FontSize', 12, 'FontWeight', 'bold');
hold off;
%% BLOC 2: DETAILED WIGGLE PLOT (5 CHANNELS, COMMON SCALE)
% -------------------------------------------------------------------------
n_traces_5 = 5;
channels_idx_5 = round(linspace(1, nx, n_traces_5));

scale_5 = 0.5 * max([abs(jdas_plot(:, channels_idx_5)), abs(haddl_plot(:, channels_idx_5))], [], 'all');

figure('Position', [100 100 1200 900], 'Color', 'w'); 
hold on;

h_in = []; h_de = [];
for i = 1:length(channels_idx_5)
    ch = channels_idx_5(i);
    h_in = plot(time, jdas_plot(:, ch)/scale_5 + i, 'k', 'LineWidth', 1.2);
    h_de = plot(time, haddl_plot(:, ch)/scale_5 + i, 'r', 'LineWidth', 1.2);
end

h_pw = xline(12.3, '--b', 'LineWidth', 1.5, 'Label', 'P-wave');
xlabel('Time (s)'); 
ylabel('Trace index');
title('DAS Wiggle Plot (Input: black, Denoised: red, Same Amplitude Scale)');
legend([h_in, h_de, h_pw], {'Input (raw)', 'Denoised', 'P-wave arrival'}, 'Location', 'northeast');
grid on; 
set(gca, 'LineWidth', 1.5, 'FontSize', 12, 'FontWeight', 'bold');
hold off;
%% BLOC 3: NORMALIZED WIGGLE PLOT (PER-TRACE NORMALIZATION)
% -------------------------------------------------------------------------
figure('Position', [100 100 1200 900], 'Color', 'w'); 
hold on;

% Dummy handles for legend initialization
...
for i = 1:length(channels_idx_5)
    ...
    % Normalize input trace safely
    max_in = max(abs(jdas_plot(:, ch)));
    if max_in == 0, max_in = 1; end
    in_trace = jdas_plot(:, ch) / max_in;
    
    % Normalize denoised trace safely
    max_de = max(abs(haddl_plot(:, ch)));
    if max_de == 0, max_de = 1; end
    de_trace = haddl_plot(:, ch) / max_de;
